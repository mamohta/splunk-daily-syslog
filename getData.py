...
parser.add_argument('--server', action='store', dest='server',help='Give server',type=str)
...
#For Splunk username and Password
fle=open("pwd.txt","r")
...
customerDf=pd.read_csv("kvstore_syslog_register_"+server+".csv")

#If it is not a Monday keep only daily cpyKey
if not "Mon" in currentDay:
	customerDf=customerDf[(customerDf['frequency']=="Daily")]
...
for index, row in customerDf.iterrows():
	...
	#Set earliest and latest on the basis of frequency
	latest=1
	if "Mon" in currentDay:
